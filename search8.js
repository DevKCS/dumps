...
function fnSetData(pageNum) {
  var sJson = {};
  var cmpNm = $("#cmpNm").value;  //기업명
  var rprsvNm = $("#rprsvNm").value; //대표자명
  var bizRNo = $("input[name=bizRNo]").value;  //사업자번호
  var pageNo = pageNum;//페이지 번호
  var pageSize = $("select[name=pageSize]").value;
  var strArea = $("select[name=sidoAreaCd]").value;
  var strSigunguArea = $("select[name=sigunguAreaCd]").value;
  var strIndsty = $("select[name=selIndsty]").value;
  if (strArea == "00") {
    strArea = "";
  }
  sJson.cmpNm = cmpNm;
  ...
  $(document).ready(function () {
    ...
    $("#sidoAreaCd").on('change', function () {
      createSelectSggList($("#sidoAreaCd").value);
    });
  });
